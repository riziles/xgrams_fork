<script lang="ts">
	...
	// };// TODO: Use as defaults
	function onToggleHandler(): void {
		// get(darklightUser).toggleForward();		// WTAF

		$darklightUser.value = Math.abs(($darklightUser.value + 2) % 3);
		setUserDarkLight($darklightUser.value);

		$darklightUser = $darklightUser;
		$currentDarklight = getDarkLightMode();
		setDarkLightCurrent($currentDarklight);
	}
	// A11y Input Handlers
	...
	// State
	// $: trackBg = $currentDarklight === true ? bgLight : bgDark;
	// $: thumbBg = $currentDarklight === true ? bgDark : bgLight;
	// $: thumbPosition = $currentDarklight === true ? 'translate-x-[100%]' : '';
	// $: iconFill = $currentDarklight === true ? fillLight : fillDark;
	// Reactive
	//	$: classesTrack = `${cTrack} ${cTransition} ${width} ${height} ${ring} ${rounded} ${trackBg} ${$$props.class ?? ''}`;
	$: classesTrack = `${cTrack} ${cTransition} ${width} ${height} ${rounded} ${$$props.class ?? ''}`;
	// $: classesThumb = `${cThumb} ${cTransition} ${height} ${rounded} ${thumbBg} ${thumbPosition}`;
	// $: classesIcon = `${cIcon} ${iconFill}`;
</script>

<svelte:head>
	{@html `<script>${initializeDarkLight.toString()} initializeDarkLight();</script>`}
</svelte:head>
<div
	class={classesTrack}
	...
	aria-checked={$currentDarklight}
	title="Toggle {$darklightUser.value == 2
		? 'Use OS'
		: $darklightUser.value == 1
		? 'Light'
		: 'Dark'} Mode"
	tabindex="0"
>
	{#if $darklightUser.value == 0}
		<slot name="dark">Missing always dark icon</slot>
	{:else if $darklightUser.value == 1}
		<slot name="light">Missing always light icon</slot>
	{:else if $darklightUser.value == 2}
